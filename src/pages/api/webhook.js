...
const fulfillOrder = async (session) => {
  ...
  return app
    ...
    .then(() => {
      console.log(
        `SUCCESS: Order ${session.id} has been added to the DB. Session: ${session}`
      );
    })
    .catch(console.log(`Failure: ${session}`));
};
      ...
      return fulfillOrder(session)
        ...
        .catch((err) => {
          return res
            .status(400)
            .send(
              `Webhook Error: ${err.message}. ${session.id} ${session.metadata.email}`
            );
        });
    }
  }
